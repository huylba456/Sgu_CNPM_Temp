...
import { products as seedProducts, categories } from '../../src/data/mockData';
...
export default function AdminProductsScreen() {
  ...
  const handleSave = () => {
    ...
    setForm({ id: '', name: '', category: categories[0], price: '0', restaurant: 'FastGrill Station' });
  };
  ...
  return (
    <Screen>
      <Header title="Admin • Sản phẩm" />
      <View style={styles.form}>
        <Text style={styles.label}>Tên món</Text>
        <TextInput style={styles.input} value={form.name} onChangeText={(value) => setForm((prev) => ({ ...prev, name: value }))} />
        <Text style={styles.label}>Giá</Text>
        <TextInput style={styles.input} keyboardType="numeric" value={form.price} onChangeText={(value) => setForm((prev) => ({ ...prev, price: value }))} />
        ...
        {products.map((product) => (
          <View key={product.id} style={styles.item}>
            <View>
              ...
              <Text style={styles.muted}>{product.category} • {product.restaurant}</Text>
            </View>
